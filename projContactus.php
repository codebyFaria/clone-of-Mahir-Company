...
if (isset($_POST['btnadd'])) {
    $name = $_POST['txtname'];
    $phone_number = $_POST['txtphone_number'];
    $message = $_POST['txtmessage']; // Correct variable name
    $screenshot = rand(0, 99999) . $_FILES['txtscreenshot']['name'];
    
    // Move the uploaded file to the desired directory
    if (move_uploaded_file($_FILES['txtscreenshot']['tmp_name'], "uphoto/" . $screenshot)) {
        // Use prepared statements to avoid SQL injection
        $stmt = $conn->prepare("INSERT INTO contactus (`txtname`, `txtphone_number`, `txtmessage`, `txtscreenshot`) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $name, $phone_number, $message, $screenshot);

        // Execute the query
        if ($stmt->execute()) {
            echo "Data inserted successfully";
        } else {
            echo "Sorry, there was an error: " . $stmt->error;
        }
...
.cl-10, .cl-4, .cl-5, .cl-7, .cl-8, .cl-12, .cl-6, .cl-3, .cl-2, .cl-1 {
    position: relative;
    width: 100%;
    padding-right: 15px;
    padding-left: 15px;
...
.form-textarea.form-field, textarea.form-field {
    height: auto;
}
.form-text {
    display: block;
...
.text-danger, .error-message, .errors-message {
    color: #fa646a;
...
.image-add-icon, .image-preview>img {
    position: absolute;
    transform: translate(-50%, -50%);
}
.image-add-icon::before, .image-add-icon::after {
    content: "";
    width: 17px;
    height: 3px;
    background: #707070;
...
<body>
                    ...
                    <li class="dropdown">
                        ...
                        <div class="menu-align" style="display: flex; justify-content: space-between; gap: 10px;">
                            <div class="buttons">
                                ...
                                <button style="border-radius: 20px" class="btn">signin/signup</button>
    ...
    <div class="container2">
        <div class="bannercontent">
            <h1 style="color: #f0f0f0; font-family:Poppins, sans-serif;font: size 2rem">Contact Us</h1>
        </div>
    </div>
    </div>
    </div>
        <div class="contained">
            ...
            <h2 class="contact-heading" style="font-family:Poppins, sans-serif ; font-size: 32px">Contact Us</h2>
            <p style="font-size: 15px;font-family:Poppins, sans-serif">If you have any query or complaint, please feel free to reach out to us by filling the form given below or email us at <a href="mailto:info@mrmahir.com">info@mrmahir.com</a>. <br>How May We Help you?</p>
            </div>
            <div class="contact-form-wrapper">
                <form name="contact-us-form" id="contact-us-form" class="contact-us-form">
                    <div class="">
                        <div class="line">
                            <div class="cl-6 field-wrapper">
                                <input type="text" name="name" id="name" class="form-field cuf-field" placeholder="Your name please">
                                <div id="name-error" class="error-message"></div>
                            </div>
                            <div class="cl-6 field-wrapper">
                                <input name="phone_number" id="phone_number" class="form-field" type="text" required="" placeholder="Please Enter Your Phone number *" style="font-family:Poppins, sans-serif ; font-size: 14px">
                                <div id="phone-error" class="error-message"></div>
                            </div>
                        </div>
                        <div class="field-wrapper">
                            <textarea style="font-family:Poppins, sans-serif" name="message" id="message" rows="5" maxlength="1500" class="form-textarea form-field" placeholder="Describe your problem" data-gramm="false" wt-ignore-input="true"></textarea> 
                            <small style="font-family:Poppins, sans-serif ; font-size: 14px" id="char-count" class="text-muted form-text"><strong id="char-count-number" class="text-danger">1500</strong> characters remaining</small>
                            <div id="message-error" class="error-message"></div>
                        </div>
                        <div class="field-wrapper">
                            <h4 style="font-family:Poppins, sans-serif ;">Add Screenshots</h4>
                            <div class="image-uploader">
                                <div id="frames"></div>
                                <div class="image-add">
                                    <input id="image-input" type="file" class="screenshot" multiple="">
                                    <span class="image-add-icon"></span>
                                </div>
                            </div>
                            <div class="error-message"></div>
                        </div>
                        <div class="form-btn-wrapper field-wrapper">
                            <button type="submit" class="big-btn btn bg-primary">Submit</button>
                        </div>
                    </div>
            </div>
            <div style="display: flex; justify-content: space-around; max-width: 70%; margin: 60px auto 0; text-align: center;">
            <div style="width:50%">
            <strong>Head Office</strong>
            <p>26-A, J 3 Block Block J 3 Phase 2 Johar Town, Lahore, Punjab 54000</p>
            </div>
            <div style="width:50%">
            <strong>Lahore Office</strong>
            <p>B-300 Glamour Height Apartments, Waris Road, Jubilee Town, Lahore, Punjab 54000</p>
            </div>
            </div>
            </div>
            </div>
       ...
       ?>
        ...
        <script>
            // JavaScript code for form validation and image upload
            document.addEventListener('DOMContentLoaded', function() {
                ...
                messageTextarea.addEventListener('input', function() {
                    const remainingChars = 1500 - this.value.length;
                    charCountNumber.textContent = remainingChars;
                });
                ...
                form.addEventListener('submit', function(event) {
                    ...
                    if (phoneInput.value.trim() === '') {
                        ...
                        document.getElementById('phone-error').textContent = 'Phone number is not a valid number';
                        isValid = false;
                    } else {
                    ...
                    if (messageTextarea.value.trim() === '') {
                        document.getElementById('message-error').textContent = 'Please describe your problem';
                        isValid = false;
                    } else if (messageTextarea.value.trim().split(/\s+/).length < 5) {
                        document.getElementById('message-error').textContent = 'Message should be more than 5 words';
                        isValid = false;
                    } else {
